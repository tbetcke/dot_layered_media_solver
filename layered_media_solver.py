...
def initialize_context(aca_eps,aca_maximumBlockSize):
    ...
    aca_ops.eps=1E-5
    aca_ops.maximumBlockSize=aca_maximumBlockSize
...
def apply_block(A,brow,bcol,x):
    """Apply the vector x to a block element of the BlockedBoundaryOperator A"""
 

    A00 = A.block(2*brow,2*bcol).weakForm()
    A01 = A.block(2*brow,2*bcol+1).weakForm()
    A10 = A.block(2*brow+1,2*bcol).weakForm()
    A11 = A.block(2*brow+1,2*bcol+1).weakForm()
...
def apply_sgs(A,diagInv,x,y,process_map):
    """ Apply SGS preconditioner """


    single_proc_map = process_map.single_proc_map
    multiple_proc_map = process_map.multiple_proc_map
    exporter = process_map.exporter
    importer = process_map.importer
    layers = process_map.layers
    scheduler = process_map.scheduler
    block_matrix_layout = process_map.block_matrix_layout
    my_id = process_map.my_id
    nproc = process_map.nproc
    ndof = A.weakForm().shape[0]
    comm = process_map.comm()

    w = Epetra.MultiVector(multiple_proc_map,x.NumVectors()) # will hold result of forward substitution
    winv = Epetra.MultiVector(multiple_proc_map,x.NumVectors())
    x_loc = Epetra.MultiVector(multiple_proc_map,x.NumVectors())
    x_loc.Import(x,importer,Epetra.Insert)
    y_loc = Epetra.MultiVector(multiple_proc_map,y.NumVectors())

    w_loc_data = w.ExtractView()
    winv_loc_data = winv.ExtractView()
    x_loc_data = x_loc.ExtractView()
    y_loc_data = y_loc.ExtractView()

    # Compute offsets of blocks
    ...
    for i in range(nb):
        ...
        block_ndof = offsets[i+1]-offsets[i]
        block_prod_loc = np.zeros((2*block_ndof,x.NumVectors()),dtype='float64')
        for j in range(i):
            if block_matrix_layout[j,i]==my_id+1: # Block is on local processor
                tmp =apply_block(A,i,j,winv_loc_data[:,offsets[j]:offsets[j+1]].T+1j*winv_loc_data[:,ndof+offsets[j]:ndof+offsets[j+1]].T)
                block_prod_loc[:block_ndof,:] += np.real(tmp)
                block_prod_loc[block_ndof:,:] += np.imag(tmp)
        # Global communication to sum all block products
        block_prod = comm.SumAll(block_prod_loc)
        block_prod_complex = block_prod[:block_ndof,:]+1j*block_prod[block_ndof:,:]

        # Subtract from the right-hand side and store in w
        w_loc_data[:,offsets[i]:offsets[i+1]] = x_loc_data[:,offsets[i]:offsets[i+1]]-np.real(block_prod_complex).T
        w_loc_data[:,ndof+offsets[i]:ndof+offsets[i+1]] = x_loc_data[:,ndof+offsets[i]:ndof+offsets[i+1]]-np.imag(block_prod_complex).T
                               
        # Now apply the inverse if the ith diagonal block.
        data = w_loc_data[:,offsets[i]:offsets[i+1]]+1j*w_loc_data[:,ndof+offsets[i]:ndof+offsets[i+1]]
        tmp_complex_loc = diagInv.apply_block(data.T,i)
        tmp_real_loc = np.vstack([np.real(tmp_complex_loc),np.imag(tmp_complex_loc)])
        tmp_real = comm.SumAll(tmp_real_loc)
        
        # Store the result in winv
        winv_loc_data[:,offsets[i]:offsets[i+1]] = tmp_real[:block_ndof,:].T
        winv_loc_data[:,ndof+offsets[i]:ndof+offsets[i+1]] = tmp_real[block_ndof:,:].T
    ...
    for i in range(nb-1,-1,-1):
        ...
        for j in range(i+1,nb):
            if block_matrix_layout[i,j]==my_id+1: # Block is on local processor
                tmp = apply_block(A,i,j,w_loc_data[:,offsets[j]:offsets[j+1]].T+1j*w_loc_data[:,ndof+offsets[j]:ndof+offsets[j+1]].T)
                block_prod_loc[:block_ndof,:] += np.real(tmp)
                block_prod_loc[block_ndof:,:] +=np.imag(tmp)
        ...
        # Subtract from rhs
        res = (w_loc_data[:,offsets[i]:offsets[i+1]]+1j*w_loc_data[:,ndof+offsets[i]:ndof+offsets[i+1]]).T-block_prod_complex

        # Apply the inverse of the diagonal block

        tmp_complex_loc = diagInv.apply_block(res,i)
        tmp_real_loc = np.vstack([np.real(tmp_complex_loc),np.imag(tmp_complex_loc)])
        tmp_real = comm.SumAll(tmp_real_loc)
        
        # Store the result in y
        y_loc_data[:,offsets[i]:offsets[i+1]] = tmp_real[:block_ndof,:].T
        y_loc_data[:,ndof+offsets[i]:ndof+offsets[i+1]] = tmp_real[block_ndof:,:].T

    # Communicate result back to y


    if my_id==0:
        y[:,:] = y_loc_data[:,:] # Works since all processors have the same copy of y_loc_data
...
class ProcessMap(object):
    ...
    @property
    def block_matrix_layout(self):
        return self.__block_matrix_layout

    @property
    def my_id(self):
        return self.__comm.MyPID()

    @property
    def nproc(self):
        return self.__comm.NumProc()
...
class SymmetricGaussSeidelPreconditioner(Epetra.Operator):
    """Provide a Symmetric Gauss Seidel Preconditioner"""

    def __init__(self,A,process_map,context,aca_tol=1E-2):
        Epetra.Operator.__init__(self)

        self.__label = "SGSPreconditioner"
        self.__A = A
        self.__comm = Epetra.PyComm()
        self.__process_map = process_map
        self.__single_proc_map = process_map.single_proc_map
        self.__context = context
        self.__aca_tol = aca_tol
        self.__block_diag_precond = BlockDiagonalPreconditioner(A,process_map,context,aca_tol)
        self.__total_time_prec_initialisation = self.__block_diag_precond.TotalTimePrecInitialisation()
    ...
    def ApplyInverse(self,x,y):
        ...
        try:
            apply_sgs(self.__A,self.__block_diag_precond,x,y,self.__process_map)
            return 0
        except Exception, e:
    ...
    def TotalTimePrecInitialisation(self):
        return self.__total_time_prec_initialisation
...
class BlockDiagonalPreconditioner(Epetra.Operator):
    ...
    def __init__(self,A,process_maps,context,aca_tol=1E-2):
        ...
        self.__scheduler = process_maps.scheduler
        self.__context = context
        self.__blocks, self.__block_ids = self.buildBlockInverses(aca_tol)
        self.__ndofs_complex=sum([self.__layers[i]['spaces']['ndof'] for i in range(len(self.__layers))])
        self.__total_time_prec_initialisation
    ...
    def apply_block(self,x,id):
        """Apply a local block to numpy array x and return result (x is complex)
           Return zero if id does not live on local processor
        """


        try:
            ind = self.__block_ids.index(id)
        except: return np.zeros_like(x)
        block = self.__blocks[ind]
        return block['lu'].operator*x
...
def solve_system(operator,rhs,process_maps,tol=1E-5,max_it=1000,prec=None):
...
class LayeredMediaModifiedHelmholtzSolver(object):

    def __init__(self,grids,graph,wavenumbers,kappa_vec,impedance,
                 tol=1E-8,max_it=1000,aca_max_block=20000,
                 aca_eps=1E-5,aca_lu_eps=1E-2,verbosity="low"):
        self.__grids = grids
        self.__graph = graph
        self.__wavenumbers = wavenumbers
        self.__kappa_vec = kappa_vec
        self.__impedance = impedance
        self.__tol = tol
        self.__max_it = max_it
        self.__aca_max_block = aca_max_block
        self.__aca_eps = aca_eps
        self.__aca_lu_eps = aca_lu_eps
        self.__verbosity = verbosity

        self.__comm = Epetra.PyComm()
        self.__my_proc = self.__comm.MyPID()
        self.__nproc = self.__comm.NumProc()
        
        self.__context = initialize_context(self.__aca_eps,self.__aca_max_block)
        self.__layers = initialize_layers(self.__context,self.__graph,self.__grids,self.__wavenumbers,self.__kappa_vec)
        self.__process_map = ProcessMap(graph,self.__layers)
        self.__assembly_time = np.zeros(self.__nproc,dtype='float')
        self.__prec_assembly_time = np.zeros(self.__nproc,dtype='float')
        self.__A = None
        self.__P = None

    def assemble(self):
        bempp_op,t_matrix = generate_local_block_operator(self.__context,self.__my_proc,self.__process_map,self.__layers,self.__impedance)
        self.__A = TrilinosOperator(bempp_op,self.__process_map)
        self.__P = SymmetricGaussSeidelPreconditioner(bempp_op,self.__process_map,self.__context,aca_tol=self.__aca_lu_eps)
        t_prec = self.__P.TotalTimePrecInitialisation()
        self.__comm.Barrier()

        self.__assembly_time[self.__my_proc] = t_matrix
        self.__prec_assembly_time[self.__my_proc] = t_prec
        self.__assembly_time=self.__comm.SumAll(self.__assembly_time)
        self.__prec_assembly_time=self.__comm.SumAll(self.__prec_assembly_time)

        if self.__my_proc==0:
            print "Matrix assembly times"
            for i in range(self.__nproc):
                print "Process: %(my_proc)i Time (s): %(tval)f" % {'my_proc':i,'tval':self.__assembly_time[i]}
            print ""
            print ""
            print "Preconditioner assembly times"
            for i in range(self.__nproc):
                print "Process: %(my_proc)i Time (s): %(tval)f" % {'my_proc':i,'tval':self.__prec_assembly_time[i]}
        
    
    def set_rhs(self,rhs_fun):
        self.__rhs = initialize_rhs(self.__context,self.__layers,rhs_fun,self.__process_map)

    def solve(self):
        if self.__my_proc==0:
            print "Starting iterative solver..."
        self.__x = solve_system(self.__A,self.__rhs,self.__process_map,tol=self.__tol,max_it=self.__max_it,prec=self.__P)
        
    def save_output(self,fname):
        save_output(self.__context,self.__layers,self.__x,fname)
...
class LayeredMediaDotSolver(LayeredMediaModifiedHelmholtzSolver):

    def __init__(self,grids,graph,mua,mus,freq,c,alpha,
                 tol=1E-8,max_it=1000,aca_max_block=20000,
                 aca_eps=1E-5,aca_lu_eps=1E-2,verbosity="low"):

        omega = 2*np.pi*freq*1E-12
        wavenumbers,kappa_vec=compute_wavenumbers_kappas(mua,mus,c,omega)
        LayeredMediaModifiedHelmholtzSolver.__init__(self,grids,graph,wavenumbers,
                                                     kappa_vec,2*alpha,tol,max_it,aca_max_block,
                                                     aca_eps,aca_lu_eps,verbosity)            
...
if __name__=="__main__":
    ...
    grids= [shapes.sphere(radius=radii[i],h=h,grid=True,) for i in range(len(radii))]
    ...
    context = initialize_context(1E-5,20000)
    layers = initialize_layers(context,graph,grids,wavenumbers,kappas)
    process_map = ProcessMap(graph,layers)
 
    comm = Epetra.PyComm()

    bempp_op,t = generate_local_block_operator(context,comm.MyPID(),process_map,layers,2*alpha)
    A = TrilinosOperator(bempp_op,process_map)
    P = BlockDiagonalPreconditioner(bempp_op,process_map,context,aca_tol=1E-2)

    single_proc_map = process_map.single_proc_map
    x = Epetra.MultiVector(single_proc_map,1)
    y = Epetra.MultiVector(single_proc_map,1)
    ...
    apply_sgs(bempp_op,P,x,y,process_map)

    solver = LayeredMediaDotSolver(grids,graph,mua,mus,freq,c,alpha)
    solver.assemble()
    solver.set_rhs(evalFun)
    solver.solve()
    solver.save_output('test')
